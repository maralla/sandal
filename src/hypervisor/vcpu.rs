...
use super::ffi::{hv_vcpu_read_sys_reg_wrapper, hv_vcpu_write_sys_reg_wrapper, HvSysReg,
                 hv_vcpu_set_vtimer_mask_wrapper, hv_vcpu_get_vtimer_mask_wrapper,
                 hv_vcpu_set_vtimer_offset_wrapper, hv_vcpu_get_vtimer_offset_wrapper};
...
impl Vcpu {
    ...
    pub fn read_exit_info(&self) -> String {
        ...
        unsafe {
            ...
            format!("Exit reason: {}, EC: 0x{:x}, Syndrome: 0x{:x}, VirtAddr: 0x{:x}, PhysAddr: 0x{:x}",
                    exit.reason, ec, syndrome, 
    ...
    pub fn read_exception_syndrome(&self) -> Result<u64> {
        ...
        unsafe {
            Ok((*self.exit_info).exception.syndrome)
        }
    }
    ...
    pub fn read_fault_address(&self) -> Result<u64> {
        ...
        unsafe {
            Ok((*self.exit_info).exception.physical_address)
        }
    }
    ...
    pub fn read_virtual_fault_address(&self) -> Result<u64> {
        ...
        unsafe {
            Ok((*self.exit_info).exception.virtual_address)
        }
    }
    ...
    pub fn force_exit(vcpu_ids: &[u32]) -> Result<()> {
        ...
        let ret = unsafe {
            hv_vcpus_exit_wrapper(vcpu_ids.as_ptr() as *mut _, vcpu_ids.len() as u32)
        };
    ...
    pub fn set_pending_interrupt(&self, int_type: u32, pending: bool) -> Result<()> {
        ...
        let ret = unsafe {
            hv_vcpu_set_pending_interrupt_wrapper(self.id, int_type, pending)
        };
    ...
    pub fn set_trap_debug_exceptions(&self, value: bool) -> Result<()> {
        ...
        let ret = unsafe {
            hv_vcpu_set_trap_debug_exceptions_wrapper(self.id, value)
        };
