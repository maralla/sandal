...
impl Vm {
    ...
    fn setup_gic() -> Result<()> {
        ...
        let ret = unsafe {
            ...
            if r != HV_SUCCESS {
                ...
                return Err(anyhow::anyhow!("Failed to set GIC distributor base: error code {}", r));
            }
            
            r = hv_gic_config_set_redistributor_base_wrapper(config, gic_redist_base);
            if r != HV_SUCCESS {
                hv_gic_config_release_wrapper(config);
                return Err(anyhow::anyhow!("Failed to set GIC redistributor base: error code {}", r));
            }
        ...
        unsafe { hv_gic_get_spi_interrupt_range_wrapper(&mut spi_base, &mut spi_count); }
    ...
    pub fn map_memory(&self, host_addr: *mut c_void, guest_addr: u64, size: usize, flags: u64) -> Result<()> {
        let ret = unsafe { hv_vm_map_wrapper(host_addr, guest_addr, size, flags) };
        
        if ret != HV_SUCCESS {
            anyhow::bail!("Failed to map memory at 0x{:x}: error code {}", guest_addr, ret);
        }
    ...
    pub fn unmap_memory(&self, guest_addr: u64, size: usize) -> Result<()> {
        ...
        if ret != HV_SUCCESS {
            anyhow::bail!("Failed to unmap memory at 0x{:x}: error code {}", guest_addr, ret);
        }
