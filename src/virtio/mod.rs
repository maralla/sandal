...
pub fn read_descriptor(memory: &[u8], ram_base: u64, desc_addr: u64, index: u16) -> Option<(u64, u32, u16, u16)> {
    let entry_addr = desc_addr + (index as u64) * 16;
    let offset = entry_addr.checked_sub(ram_base)? as usize;
    if offset + 16 > memory.len() { return None; }
...
pub fn read_avail_idx(memory: &[u8], ram_base: u64, avail_addr: u64) -> Option<u16> {
    ...
    if offset + 2 > memory.len() { return None; }
    Some(u16::from_le_bytes(memory[offset..offset+2].try_into().ok()?))
}

/// Read an entry from the available ring
pub fn read_avail_ring(memory: &[u8], ram_base: u64, avail_addr: u64, idx: u16, queue_size: u32) -> Option<u16> {
...
pub fn write_used_ring(memory: &mut [u8], ram_base: u64, used_addr: u64, used_idx: u16, queue_size: u32, desc_id: u32, len: u32) -> Option<()> {
    ...
    if offset + 8 > memory.len() { return None; }
